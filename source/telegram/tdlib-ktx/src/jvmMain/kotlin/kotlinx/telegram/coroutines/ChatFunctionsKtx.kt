...
suspend fun TelegramFlow.createChatFolderInviteLink(
    ...
    name: String?,
    chatIds: LongArray?,
): ChatFolderInviteLink = this.sendFunctionAsync(
    TdApi.CreateChatFolderInviteLink(
        chatFolderId,
        name, chatIds
    )
)
...
): GroupCallId = this.sendFunctionAsync(
    TdApi.CreateVideoChat(
        chatId, title, startDate,
        isRtmpStream
    )
)
...
suspend fun TelegramFlow.editChatFolderInviteLink(
    ...
    TdApi.EditChatFolderInviteLink(
        chatFolderId,
...
/**
 * Suspend function, which returns list of features available on the first 10 chat boost levels;
...
/**
 * Suspend function, which returns list of features available on the specific chat boost level; this
...
suspend fun TelegramFlow.getChatBoostLevelFeatures(isChannel: Boolean, level: Int):
        ChatBoostLevelFeatures = this.sendFunctionAsync(
    TdApi.GetChatBoostLevelFeatures(
        isChannel,
        level
    )
)
...
suspend fun TelegramFlow.getChatBoosts(
    ...
    offset: String?,
    limit: Int,
): FoundChatBoosts = this.sendFunctionAsync(
    TdApi.GetChatBoosts(
        chatId, onlyGiftCodes, offset,
        limit
    )
)
...
suspend fun TelegramFlow.getChatMessageCount(
    ...
    savedMessagesTopicId: Long,
    returnLocal: Boolean,
): Count = this.sendFunctionAsync(
    TdApi.GetChatMessageCount(
        chatId, filter, savedMessagesTopicId,
        returnLocal
    )
)
...
suspend fun TelegramFlow.getChatMessagePosition(
    ...
    TdApi.GetChatMessagePosition(
        chatId, messageId, filter,
        messageThreadId, savedMessagesTopicId
    )
)

/**
 * Suspend function, which returns list of chats with non-default notification settings for new
 * messages.
 *
...
/**
 * Suspend function, which returns the list of pinned stories posted by the given chat. The stories
 * are returned in a reverse chronological order (i.e., in order of decreasing storyId). For optimal
 ...
 * @return [Stories] Represents a list of stories.
 */
suspend fun TelegramFlow.getChatPinnedStories(
    chatId: Long,
    fromStoryId: Int,
    limit: Int,
): Stories = this.sendFunctionAsync(TdApi.GetChatPinnedStories(chatId, fromStoryId, limit))
...
suspend fun TelegramFlow.getChatStoryInteractions(
    ...
    offset: String?,
    limit: Int,
): StoryInteractions = this.sendFunctionAsync(
    TdApi.GetChatStoryInteractions(
        storySenderChatId,
...
suspend fun TelegramFlow.searchChatMessages(
    ...
    messageThreadId: Long,
    savedMessagesTopicId: Long,
): FoundChatMessages = this.sendFunctionAsync(
...
suspend fun TelegramFlow.sendChatAction(
    ...
    TdApi.SendChatAction(
        chatId, messageThreadId, businessConnectionId,
        action
    )
)
...
suspend fun TelegramFlow.setChatAccentColor(
    ...
    TdApi.SetChatAccentColor(
        chatId, accentColorId,
 ...
 */
suspend fun TelegramFlow.setChatAvailableReactions(
    chatId: Long,
...
suspend fun TelegramFlow.setChatBackground(
    ...
    TdApi.SetChatBackground(
        chatId, background, type, darkThemeDimming,
        onlyForSelf
    )
)
...
suspend fun TelegramFlow.setChatProfileAccentColor(
    ...
    TdApi.SetChatProfileAccentColor(
        chatId, profileAccentColorId,
...
suspend fun TelegramFlow.shareChatWithBot(
    ...
    TdApi.ShareChatWithBot(
        chatId, messageId, buttonId, sharedChatId,
        onlyCheck
    )
)
