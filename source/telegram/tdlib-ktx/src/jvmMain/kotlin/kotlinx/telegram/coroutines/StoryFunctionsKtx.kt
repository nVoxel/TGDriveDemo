...
/**
 * Suspend function, which returns list of chats with non-default notification settings for stories.
 *
...
suspend fun TelegramFlow.getStoryPublicForwards(
    ...
    TdApi.GetStoryPublicForwards(
        storySenderChatId, storyId,
        offset, limit
    )
)
...
): Story = this.sendFunctionAsync(
    TdApi.SendStory(
        chatId, content, areas, caption, privacySettings,
        activePeriod, fromStoryFullId, isPinned, protectContent
    )
)
 ...
 */
suspend fun TelegramFlow.setStoryPrivacySettings(
    storyId: Int,
    privacySettings: StoryPrivacySettings?,
) =
...
suspend fun TelegramFlow.setStoryReaction(
    ...
    TdApi.SetStoryReaction(
        storySenderChatId, storyId, reactionType,
 ...
 * only if story.canToggleIsPinned == true.
 *
 * @param storySenderChatId Identifier of the chat that posted the story.
 * @param storyId Identifier of the story.
 * @param isPinned Pass true to make the story accessible after expiration; pass false to make it
 * private.
 */
suspend fun TelegramFlow.toggleStoryIsPinned(
    storySenderChatId: Long,
    storyId: Int,
    isPinned: Boolean,
) = this.sendFunctionLaunch(TdApi.ToggleStoryIsPinned(storySenderChatId, storyId, isPinned))
