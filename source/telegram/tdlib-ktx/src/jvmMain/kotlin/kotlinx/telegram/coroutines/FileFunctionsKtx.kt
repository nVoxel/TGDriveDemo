...
suspend fun TelegramFlow.getFileDownloadedPrefixSize(fileId: Int, offset: Long):
        FileDownloadedPrefixSize = this.sendFunctionAsync(
...
suspend fun TelegramFlow.getMapThumbnailFile(
    ...
    scale: Int,
    chatId: Long,
): File = this.sendFunctionAsync(
    TdApi.GetMapThumbnailFile(
        location, zoom, width, height, scale,
        chatId
    )
)
 ...
 * have a persistent remote identifier until it is sent in a message.
 *
...
suspend fun TelegramFlow.searchFileDownloads(
    ...
    offset: String?,
    limit: Int,
): FoundFileDownloads = this.sendFunctionAsync(
    TdApi.SearchFileDownloads(
        query, onlyActive,
