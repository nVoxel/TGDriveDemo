...
import kotlinx.telegram.coroutines.getChatPinnedStories
...
interface ChatKtx : BaseKtx {
     ...
     * used to join a chat. Members can't be added to a channel if it has more than 200 members.
     *
     ...
     */
    suspend fun Chat.clickSponsoredMessage(messageId: Long) = api.clickChatSponsoredMessage(
        this.id,
        messageId
    )
     ...
     */
    suspend fun Chat.deleteBackground(restorePrevious: Boolean) = api.deleteChatBackground(
        this.id,
        restorePrevious
    )
     ...
     */
    suspend fun Chat.deleteForumTopic(messageThreadId: Long) = api.deleteForumTopic(
        this.id,
        messageThreadId
    )
    ...
    suspend fun Chat.forwardMessages(
        ...
        this.id, messageThreadId, fromChatId, messageIds, options, sendCopy,
        removeCaption
    )
     ...
     * in the result.
     *
     * @return [TdApi.Emojis] Represents a list of emoji.
     */
    ...
    /**
     * Suspend function, which returns list of boosts applied to a chat; requires administrator rights
     * in the chat.
     *
    ...
    /**
     * Suspend function, which returns the list of pinned stories posted by the given chat. The
     * stories are returned in a reverse chronological order (i.e., in order of decreasing storyId). For
     ...
     * @return [TdApi.Stories] Represents a list of stories.
     */
    suspend fun Chat.getPinnedStories(fromStoryId: Int, limit: Int) =
        api.getChatPinnedStories(this.id, fromStoryId, limit)
     ...
     * @return [TdApi.Count] Contains a counter.
     */
    suspend fun Chat.getSimilarCount(returnLocal: Boolean) = api.getChatSimilarChatCount(
        this.id,
        returnLocal
    )
     ...
     */
    suspend fun Chat.getForumTopic(messageThreadId: Long) = api.getForumTopic(
        this.id,
        messageThreadId
    )
     ...
     * a forum topic.
     */
    suspend fun Chat.getForumTopicLink(messageThreadId: Long) = api.getForumTopicLink(
        this.id,
        messageThreadId
    )
     ...
     */
    suspend fun Chat.getPremiumGiveawayInfo(messageId: Long) = api.getPremiumGiveawayInfo(
        this.id,
        messageId
    )
    ...
    /**
     * Suspend function, which returns list of boosts applied to a chat by a given user; requires
    ...
    suspend fun Chat.getWebAppLinkUrl(
        ...
        allowWriteAccess: Boolean,
    ) = api.getWebAppLinkUrl(
        this.id, botUserId, webAppShortName, startParameter, theme,
     ...
     * The method is independent from openChat and closeChat methods.
     *
    ...
    suspend fun Chat.sendBusinessMessage(
        ...
        businessConnectionId, this.id, replyTo, disableNotification,
        protectContent, replyMarkup, inputMessageContent
    )
    ...
    suspend fun Chat.sendBusinessMessageAlbum(
        ...
        businessConnectionId, this.id, replyTo, disableNotification,
        protectContent, inputMessageContents
    )
    ...
    suspend fun Chat.sendInlineQueryResultMessage(
        ...
        this.id, messageThreadId, replyTo, options, queryId,
        resultId, hideViaBot
    )
    ...
    suspend fun Chat.sendStory(
        ...
        this.id, content, areas, caption, privacySettings, activePeriod,
        fromStoryFullId, isPinned, protectContent
    )
     ...
     */
    suspend fun Chat.setEmojiStatus(emojiStatus: EmojiStatus?) = api.setChatEmojiStatus(
        this.id,
        emojiStatus
    )
     ...
     */
    suspend fun Chat.setProfileAccentColor(
        profileAccentColorId: Int,
        profileBackgroundCustomEmojiId: Long,
    ) = api.setChatProfileAccentColor(
        this.id,
     ...
     */
    suspend fun Chat.setForumTopicNotificationSettings(
        messageThreadId: Long,
     ...
     */
    suspend fun Chat.toggleViewAsTopics(viewAsTopics: Boolean) = api.toggleChatViewAsTopics(
        this.id,
        viewAsTopics
    )
