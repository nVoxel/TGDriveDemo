...
interface FileKtx : BaseKtx {
     ...
     */
    suspend fun File.cancelDownload(onlyIfPending: Boolean) = api.cancelDownloadFile(
        this.id,
        onlyIfPending
    )
     ...
     */
    suspend fun File.getDownloadedPrefixSize(offset: Long) = api.getFileDownloadedPrefixSize(
        this.id,
        offset
    )
     ...
     */
    suspend fun File.getSuggestedName(directory: String?) = api.getSuggestedFileName(
        this.id,
        directory
    )
     ...
     */
    suspend fun File.toggleDownloadIsPaused(isPaused: Boolean) = api.toggleDownloadIsPaused(
        this.id,
        isPaused
    )
}
